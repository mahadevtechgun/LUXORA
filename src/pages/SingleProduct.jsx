...
function SingleProduct() {
      ...
      try {
        ...
        const productSizes =
          singleProduct?.attributes?.find(
            (attr) =>
              attr.name === "Size" ||
              attr.name === "SelectSize" ||
              attr.name === "Select Size"
          )?.terms || [];
        ...
        if (productSizes.length > 0) {
          setSelectedSize(productSizes[0].name);
        }
      } catch (error) {
        console.log(error);
      }
  ...
  const colors =
    product?.attributes?.find((attr) => attr.name === "Color")?.terms || [];
  ...
  const discountText = OffProduct({ items: product });
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#fbfaf7] to-[#f7f5f0]">
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 sm:py-24 lg:py-32">
        <div className="flex flex-wrap items-center gap-1 text-xs sm:text-sm font-medium text-neutral-500 mb-6 sm:mb-8">
          <Link to="/" className="hover:text-[#9b7423] transition-colors whitespace-nowrap">
            Home
          </Link>

          <FiChevronRight size={12} className="flex-shrink-0" />

          <Link to="/shop" className="hover:text-[#9b7423] transition-colors whitespace-nowrap">
            Shop
          </Link>

          <FiChevronRight size={12} className="flex-shrink-0" />

          <Link to="/product" className="hover:text-[#9b7423] transition-colors whitespace-nowrap">
            Product
          </Link>
        ...
        <section className="grid lg:grid-cols-2 gap-8 md:gap-12 lg:gap-14 items-start">
          ...
          <div className="space-y-6">
              ...
              >
                <span
                  className={`w-2 h-2 rounded-full ${
                    product?.is_in_stock ? "bg-green-700" : "bg-red-700"
                  }`}
                ></span>
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              ...
              <div className="flex-1">
                <Link to="/checkout">
                  <button className="w-full py-3 px-4 rounded-xl font-bold text-white bg-gradient-to-r from-black to-gray-800 hover:from-[#D6BA72] hover:to-[#c5a455] hover:text-black transition-all duration-300 transform hover:scale-[1.02] shadow-lg">
                    Buy Now
                  </button>
                </Link>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-6">
              <div className="bg-white rounded-2xl p-3 shadow-md border border-black/5 hover:shadow-lg transition-shadow flex items-center gap-3">
